...
class contrans:
    ...
    def get_crosswalk(self):
        ...
        cols_to_keep = ['bioname', 'chamber', 'party_code', 'nominate_dim1', 'icpsr', 'state_abbrev', 'district_code', 'bioguide_id']
        ...
        ideaology.to_csv('data/ideology.csv', index=False)
    ...
    def get_vote_similarity_data(self):
        ...
        vote_compare = pd.merge(votes, votes, on=['chamber', 'rollnumber'], how='inner')
        ...
        vote_compare = pd.merge(vote_compare, crosswalk, how='inner', left_on='icpsr_x', right_on='icpsr')
        vote_compare = vote_compare[['bioname', 'icpsr_y', 'agree']]
        vote_compare = pd.merge(vote_compare, crosswalk, how='inner', left_on='icpsr_y', right_on='icpsr')
